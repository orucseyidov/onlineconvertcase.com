...
class Languages extends Gopanel
{
	...
	public function add()
	{
		...
		if (isset($_POST['token'])) {
			...
			$_POST['flag'] = file_upload($_FILES['flag'], '/uploads/images/' . $this->table . '/', $img);
	...
	public function edit()
	{
		...
		if (isset($_POST['token'])) {
			...
			if (isset($_FILES['flag']) && strlen($_FILES['flag']['name']) > 1) {
				$img = seflink($_POST['name']);
				$_POST['flag'] = image_upload($_FILES['flag'], '/uploads/images/' . $this->table . '/', $img);
			} else {
				unset($_POST['flag']);
			}
